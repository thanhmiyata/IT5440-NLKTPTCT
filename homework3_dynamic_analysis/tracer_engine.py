...
class TraceEvent:
    ...
    memory_access: List[str] = field(
        default_factory=list)  # ['read:x', 'write:y']
...
class Tracer:
    ...
    def trace_function(self, frame, event, arg):
        ...
        # Skip tracing of system files and tracer engine methods
        # Only skip files that are part of the Python standard library or built-ins
        if (filename.startswith('<') and filename.endswith('>')) or \
           (filename.endswith('tracer_engine.py') and function_name in ['trace_function', 'stop_trace', '_detect_memory_access']):
